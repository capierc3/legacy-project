...
public class AppUsers implements UserList {
	...
	User user = new StudentImpl("Johnny Karate", "JK", "student", "Password", Belt.WHITE, defaultPic,
		new ArrayList<>(), new ClassListImpl(new ArrayList<>()));
	UserImpl user1 = new OwnerImpl("Fancy Nancy", "admin001", "admin", "Password", Belt.BLACK3, defaultPic,
		new ArrayList<>(), new ClassListImpl(new ArrayList<>()));
	Trainer user2 = new TrainerImpl("Country Mac", "CMac", "trainer", "Password", Belt.BLACK3, mac,
		new ArrayList<>(), new ClassListImpl(new ArrayList<>()));
	user2.setDescription("I gave him an ocular patdown, assessed the threat level, clocked a knife in his boot.");
	Trainer trainer1 = new TrainerImpl("Justic Oliver", Util.generateId(), "Joliver", "Password", Belt.BLUE,
		defaultPic, new ArrayList<>(), new ClassListImpl(new ArrayList<>()));
	trainer1.setDescription("I use ketchup on everything!");
	Trainer trainer2 = new TrainerImpl("Cameron Howe", Util.generateId(), "Chowe", "Password", Belt.GREEN_STRIPE,
		cameron, new ArrayList<>(), new ClassListImpl(new ArrayList<>()));
	trainer2.setDescription("What the hell is this Yahoo");
	Trainer trainer3 = new TrainerImpl("Joe MacMillan", Util.generateId(), "JMac", "Password", Belt.ORANGE, joe,
		new ArrayList<>(), new ClassListImpl(new ArrayList<>()));
	trainer3.setDescription("The thing that gets you to the thing");
	Trainer trainer4 = new TrainerImpl("Gordon Clark", Util.generateId(), "Gclark", "Password", Belt.BLACK2, gordon,
		new ArrayList<>(), new ClassListImpl(new ArrayList<>()));
	trainer4.setDescription("We had a problem. Now we have a product.");
	Trainer trainer5 = new TrainerImpl("Donna Clark", Util.generateId(), "Dclark", "Password", Belt.BLACK3, donna,
		new ArrayList<>(), new ClassListImpl(new ArrayList<>()));
	trainer5.setDescription("Software comes and goes. Hardware is forever.");
	Trainer trainer6 = new TrainerImpl("John Bosworth", Util.generateId(), "JBos", "Password", Belt.WHITE, john,
		new ArrayList<>(), new ClassListImpl(new ArrayList<>()));
