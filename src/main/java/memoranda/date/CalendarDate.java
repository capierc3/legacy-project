 ...
 *
 * WhiteBox test notes: Added the ability to add hours and mins for a 12 hour
 * clock; new CalendarDate(day,month,year,hour,min,isAM) A all day event would
 * just be new CalendarDate(day,month,year) Example: CalendarDate class1 = new
 * CalendarDate(4,10,2020,3,30,false) = 4/10/2020 3:30 PM CalendarDate
 * availability = new CalendarDate(4,10,2020) = all day on 4/10/2020 You should
 * be able to find if one gym class is before or after another by comparing
 * start times of the class. Example: class1.before(class2) class1.after(class2)
 * Or if a class is during another class; class1.inPeriod(class2Start,class2End)
 * All comparable should work with or without time of day set; if a class is
 * before or after an all day date class1.equals(availability) -> or if class is
